...
\begin{itemize}
	...
	\item
		...
		透過拉格朗日乘子，並求導 \(\mathbf{S}\)的特徵向量，即為 \(\mathbf{v}\) 。
	      \begin{gather}
		      \label{eqn:PCALagrange}
			  L(\mathbf{v},\lambda) = \mathbf{v^TSv}-\lambda(\mathbf{v^Tv}-1)
\\
		      \label{eqn:PCALagrange}
			  \frac{\partial L(\mathbf{v},\lambda)}{\partial\mathbf{v}}=2\mathbf{Sv}-2\lambda\mathbf{Sv} \Rightarrow \mathbf{Sv} =\lambda\mathbf{v}
	      \end{gather}
